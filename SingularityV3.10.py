...
def ColCyc(Input): 
    '''A Function that Cycles the RGB Spectrum in increments of ColorStep.
    Returns a Hex value. Input must be a Hex or RGB tuple with FULL saturation.'''
    Red = 0
    Green = 0
    Blue = 0
    ...
    if string == 'ProblemDecay':
        ...
        for x in range(len(ProgressBars)):
            if (ProgressBars[n1])[0] == 'ProblemTrigger':
                Return = ColCyc(((float(Time)-float((ProgressBars[n1])[2])))/(ProgressBars[n1])[3])
            n1 = n1 + 1
...
def DrawMaster():
    ...
    for x in range(len(ProgressBars)):
                if (ProgressBars[n1])[0] == 'ProblemTrigger':
                    A = float((ProgressBars[n1])[2]) #Event
                    t = float(Time) #Time
                    d = float((ProgressBars[n1])[3]) #Original Delay
                    Part1 = ((t-A)/d)
                    Part2 = ((A-t)/d)
                    Mag = abs(Part1) + abs(Part2)
