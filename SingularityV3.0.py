...
def StartAll():
    Scorekeeper('CLEAR',0)
    BarAdd('Energy',1,str(250)) #Title, Magnitude, Next Tick, Tick Delay
    BarAdd('MaxEnergy',1,str(1000))
...
def Progressor():
    ...
    for x in range(len(ProgressBars)):
        if int((ProgressBars[n2])[2]) < Time:
            ...
            del ProgressBars[n2]
            #print [int(OldTick),OldDelay]
            ProgressBars.append([OldTitle,Mag,int(OldTick)+int(OldDelay),OldDelay])
            #print ProgressBars
        n2 = n2 + 1
...
def BarAdd(string, magnitude, delay): #Create a new progress bar
    global ProgressBars
    global Time
    ProgressBars.append([str(string),magnitude, str(int(Time)+int(delay)),delay])
...
def DrawServers():
    ...
    for x in range(4):
        ...
        for x in range(6):
            ...
            Width = Width * n1
            c.create_rectangle((Width, Height, Width+50, Height+50),fill="black",outline='white',tag=(''.join(['Server',AlphaRelate(n3)])))
            c.create_text((Width+15, Height+20),text=(AlphaRelate(n3)), font=('Inhuman BB', 36), fill='white', justify='left',anchor='w',tag=(''.join(['ServerText',AlphaRelate(n3)])))
            n3 = n3 + 1
            n1 = n1 + 1
    n2 = n2 + 1
    n1 = 1
...
def DrawMaster():
    ...
    c.create_rectangle((int(ShipRoot[0])+PlayerSize, int(ShipRoot[1])+PlayerSize, int(ShipRoot[0])-PlayerSize, int(ShipRoot[1])-PlayerSize),fill="green",outline='green',tag='ship')
    #Draw Text
    c.create_text((CanvasHeight*0.01,CanvasHeight/1.03),text=(''.join(["Energy: ",str(Energy),'/',str(MaxEnergy)])), font=('Inhuman BB', 24), fill='white', justify='left',anchor='w')
