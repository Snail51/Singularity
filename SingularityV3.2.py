...
def DrawServers():
    ...
    for x in range(4):
        ...
        for x in range(6):
            ...
            Width = Width * n1
            c.create_rectangle((Width+Jitter(), Height+Jitter(), Width+50+Jitter(), Height+50+Jitter()),fill="black",outline='white',tag=(''.join(['Server',AlphaRelate(n3)])))
            c.create_text((Width+25+Jitter(), Height+5+Jitter()),text=(AlphaRelate(n3)), font=('Inhuman BB', 36), fill='white', justify='center',anchor='n',tag=(''.join(['ServerText',AlphaRelate(n3)])))
            n3 = n3 + 1
            n1 = n1 + 1
    n2 = n2 + 1
    n1 = 1
...
def Jitter():
    global JitterRate
    pointer = random.randint(1,JitterRate)
    if pointer == JitterRate:
...
def DrawMaster():
    ...
    #Draw Player
    c.delete('ship')
    c.create_line((int(ShipRoot[0])+Jitter(), int(ShipRoot[1])+PlayerSize, int(ShipRoot[0])+Jitter(), int(ShipRoot[1])-PlayerSize),fill="red",tag='ship')
    c.create_line((int(ShipRoot[0])-PlayerSize, int(ShipRoot[1])+Jitter(), int(ShipRoot[0])+PlayerSize, int(ShipRoot[1])+Jitter()),fill="red",tag='ship')
    c.create_oval(((int(ShipRoot[0])-PlayerSize/1.5), (int(ShipRoot[1])-PlayerSize/1.5), (int(ShipRoot[0])+PlayerSize/1.5), (int(ShipRoot[1])+PlayerSize/1.5)),outline='red')
    c.create_text(((int(ShipRoot[0])+Jitter()*50), (int(ShipRoot[1]))+Jitter()*50),fill='red',text=str(AlphaRelate(random.randint(1,26))),font=('Inhuman BB', 12))
    #Draw Text
    c.create_text((((CanvasHeight*0.01)+Jitter()),((CanvasHeight/1.03)+Jitter())),text=(''.join(["Energy: ",str(Energy),'/',str(MaxEnergy)])), font=('Inhuman BB', 24), fill='white', justify='left',anchor='w')
