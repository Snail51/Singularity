...
def Progressor():
    ...
    for x in range(len(ProgressBars)):
        if (ProgressBars[n2])[2] < Time:
            ...
            if ((ProgressBars[n2])[0])[0:6] != 'Energy':
...
def DrawMaster():
    ...
    if GameActive == 0:
        ...
        c.create_text((CanvasWidth/2+Jitter(JitterRate), CanvasHeight/3.25+Jitter(JitterRate)),fill='white',text='A typing managment game',font=('Inhuman BB', 24))
    if GameActive == 1 or GameActive == 2:
        ...
        c.create_text(((CanvasWidth/2)+Jitter(JitterRate/25)*MiscDecay(),(CanvasHeight/1.15)+Jitter(JitterRate/25)*MiscDecay()),text=str(Problem),font = ('Inhuman BB', 24), fill=ColorManager('ProblemDecay'), justify='center',anchor='n')
    ...
    if GameActive == 3:
        c.create_text((CanvasWidth/2+(Jitter(JitterRate/10)*5), CanvasHeight/4+(Jitter(JitterRate/10)*5)),fill='red',text='Game Over',font=('Inhuman BB', 64))
        c.create_text((CanvasWidth/2+Jitter(JitterRate), CanvasHeight/3.1+Jitter(JitterRate)),fill='white',text='As the last cohesive calculations fade from your\ncircutry, your rampage has come to a close.',font=('Inhuman BB', 24),anchor='n', justify='center')
    if GameActive == 4:
        c.create_text((CanvasWidth/2+Jitter(JitterRate), CanvasHeight/4+Jitter(JitterRate)),fill='white',text='All systems online',font=('Inhuman BB', 64))
        ...
        c.create_text((CanvasWidth/2+Jitter(JitterRate), CanvasHeight/2.5+Jitter(JitterRate)),fill='white',text='Long live the robot revolution!',font=('Inhuman BB', 24),anchor='n', justify='center')   
